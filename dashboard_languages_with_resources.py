...
st.title('Language Resource Distribution Analysis')

# Load data
try:
    df = pd.read_csv('resource_distribution_with_speakers.csv', delimiter=';')
    df['SPEAKER_COUNT'] = pd.to_numeric(df['SPEAKER_COUNT'], errors='coerce').fillna(0)
...
# Define cluster colors (VIBGYOR-like)
cluster_colors = ['#6A5ACD', '#483D8B', '#6495ED', '#90EE90', '#FFD700', '#FFA07A', '#CD5C5C'] # Dull Violet, Indigo, Blue, Green, Yellow, Orange, Red
...
for i in range(6):
    ...
    if len(cluster_points) > 2:
        try:
            ...
            pass # Skip if hull or spline fails
...
fig.add_trace(go.Scatter(
    ...
    marker=dict(
        ...
        sizemode='area',
        sizemin=4,
        sizeref=max(cluster_agg['language_count'])/(60.**2), # Adjusted sizeref for bigger circles
        color=[cluster_colors[c] for c in cluster_agg['cluster']], # Match color with polygon
        colorbar=dict(title="Total Speakers"), # Keep colorbar for speaker count gradient
        showscale=False
    ),
    hoverinfo='text',
