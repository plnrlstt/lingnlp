...
tab1, tab2, tab3, tab4 = st.tabs(["All Languages", "Languages with Resources", "Taxonomy Comparison (Full Names)", "Taxonomy Comparison (Tags)"])

with tab1:
    st.header("All Languages Dashboard")
...
with tab2:
    st.header("Languages with Resources Dashboard")
...
@st.cache_data
def run_gmm_analysis(file_path, file_name):
    """
    ...
    try:
        if "original" in file_name:
            ...
            df = pd.read_csv(file_path, header=None, names=['language', 'cluster'])
            data = df['cluster']
    except FileNotFoundError:
    ...
    ax.plot(n_components, aic_scores, label='AIC')
    ax.set_title(f'AIC and BIC for {file_name}')
    ax.set_xlabel('Number of components')
    ax.set_ylabel('Score')
    ax.legend(loc='best')

    return fig, optimal_n_bic, optimal_n_aic

with st.expander("GMM Analysis"):
    st.header("GMM Analysis")

    st.subheader("Taxonomy Original Fullnames")
    fig_original, bic_original, aic_original = run_gmm_analysis("c:\\Users\\kpsst\\Desktop\\Uni\\Linguistics in Modern NLP\\LingNLP\\taxonomy_original_fullnames.csv", "taxonomy_original_fullnames.csv")
    if fig_original:
        st.pyplot(fig_original)
        st.write(f"Optimal number of clusters (BIC): {bic_original}")
        st.write(f"Optimal number of clusters (AIC): {aic_original}")

    st.subheader("Taxonomy All 7500 Fullnames")
    fig_all7500, bic_all7500, aic_all7500 = run_gmm_analysis("c:\\Users\\kpsst\\Desktop\\Uni\\Linguistics in Modern NLP\\LingNLP\\taxonomy_all7500_fullnames.csv", "taxonomy_all7500_fullnames.csv")
    if fig_all7500:
        st.pyplot(fig_all7500)
        st.write(f"Optimal number of clusters (BIC): {bic_all7500}")
        st.write(f"Optimal number of clusters (AIC): {aic_all7500}")
